...
void BatchRenamer::work(ProgressDialog *)
{
        ...
        if (undo)
            if (dst.isLocalFile() && src.isLocalFile()) {
                ...
                (*tundo) << "echo \"" << dst.fileName()
                         << " -> " << src.fileName() << "\"" << Qt::endl;
                (*tundo) << "mv -f \"" << m_files[i].dst.name
                         << "\" \"" << m_files[i].src.name << "\"" << Qt::endl;
            } else {
    ...
    if (undo) {
        (*tundo) << Qt::endl << "echo \"Finished undoing " << m_files.count() << " actions.\"" << Qt::endl;
        delete tundo;
        fundo->close();
...
void BatchRenamer::writeUndoScript(QTextStream *t)
{
    ...
    (*t) << "#!/bin/sh" << Qt::endl
         << "# KRename Undo Script" << Qt::endl << "#" << Qt::endl
         << "# KRename was written by:" << Qt::endl
         << "# Dominik Seichter <domseichter@web.de>" << Qt::endl
         << "# https://www.krename.net/" << Qt::endl << "#" << Qt::endl
         << "# Script generated by KRename Version: " << VERSION << Qt::endl << Qt::endl
         << "# This script must be started with the option --krename to work!" << Qt::endl;

    // write functions:
    (*t) << "echo \"KRename Undo Script\"" << Qt::endl
         << "echo \"https://www.krename.net/\"" << Qt::endl
         << "echo \"\"" << Qt::endl;

    (*t) << "if test --krename = $1 ; then" << Qt::endl
         << "   echo \"\"" << Qt::endl
         << "else" << Qt::endl
         << "   echo \"You have to start this script\"" << Qt::endl
         << "   echo \"with the command line option\"" << Qt::endl
         << "   echo \"--krename\"" << Qt::endl
         << "   echo \"to undo a rename operation.\"" << Qt::endl
         << "   exit" << Qt::endl
         << "fi" << Qt::endl;
}
